...
* @example 
**/
int MqttConnectMqttAck(uint8_t *buff)
{
...
unsigned char mqtt_sta = 0;

extern void Read_PM(uint8_t *valueBuffer,int len);
...
void ESP8266_StaTcpClient_UnvarnishTest ( void )
{
  ...
  //uint16_t i;
		...
		{
			Read_PM((uint8_t*)strUSART_Fram_Record .Data_RX_BUF,strUSART_Fram_Record .InfBit .FramLength);
...
//			}
		...
		{  
			if(MQTT_ConnectAnalysis((uint8_t*)strEsp8266_Fram_Record .Data_RX_BUF,strEsp8266_Fram_Record .InfBit .FramLength))
			{
				mqtt_sta = 1;
			}
...
//				 USART_SendData( DEBUG_USARTx ,strEsp8266_Fram_Record .Data_RX_BUF[i]);    //转发给串口
//			}
